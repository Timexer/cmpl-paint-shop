...
export const Sidebar: React.FC<SidebarProps> = ({ scenario, completedChecks, onCopyPhrase }) => {
    return (
        <div className="w-full md:w-1/3 bg-white border-r border-gray-200 flex flex-col h-1/2 md:h-full overflow-hidden">
            {/* Objectives */}
            <div className="p-4 bg-orange-50 border-b border-orange-100">
                <h3 className="font-bold text-orange-800 mb-2 text-sm uppercase flex items-center">
                        ...
                        scenario.checklists.map(item => (
                            <div
                                ...
                                className={clsx(
                                    ...
                                    completedChecks[item.id] ? "text-green-600 font-bold" : "text-gray-600"
                                )}
                            >
            ...
            {/* Phrase Book */}
            <div className="p-3 bg-gray-100 border-b border-gray-200 font-semibold text-gray-700 flex justify-between items-center">
                <span className="flex items-center"><BookOpen className="w-4 h-4 mr-2" />Phrase Book (A1/A2)</span>
            </div>

            <div className="flex-1 overflow-y-auto p-4 space-y-6">
                <div className="mb-4">
                    <h3 className="font-bold text-gray-700 mb-2 text-sm uppercase border-b pb-1">General Phrases</h3>
                ...
                {scenario && (
                    <div className="bg-orange-50 rounded-lg p-3 border border-orange-100 mb-4">
                        <h3 className="font-bold text-orange-800 mb-2 text-sm uppercase">Scenario: {scenario.name}</h3>
                        <ul className="space-y-2 text-sm text-gray-700">
                            <li className="text-xs text-gray-500 italic">Use the chat to get info, then use email to confirm.</li>
                        </ul>
                    </div>
                )}

                <div>
                    <h3 className="font-bold text-gray-700 mb-2 text-sm uppercase border-b pb-1">Car Terminology</h3>
...
const PhraseItem = ({ text, sub, onCopy }: { text: string, sub: string, onCopy: (t: string) => void }) => (
    <li
        className="group cursor-pointer hover:text-orange-600 transition flex flex-col items-start"
        onClick={() => onCopy(text.replace(/"/g, ''))}
    >
        <span className="flex items-center">
            {text} <Copy className="w-3 h-3 ml-2 opacity-0 group-hover:opacity-100 transition-opacity" />
        </span>
        <span className="text-xs text-gray-400 italic">{sub}</span>
    </li>
);
...
const TermItem = ({ term, sub }: { term: string, sub: string }) => (
    <div>
        ...
        <span className="text-gray-400 italic block ml-1">{sub}</span>
    </div>
);
